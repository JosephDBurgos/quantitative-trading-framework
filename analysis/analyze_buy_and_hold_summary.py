...
def analyze_buy_and_hold_summary():
    PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    RESULTS_DIR = os.path.join(PROJECT_ROOT, "results")
    STRATEGY_DIR = os.path.join(RESULTS_DIR, "buy_and_hold")
    os.makedirs(STRATEGY_DIR, exist_ok=True)
    SUMMARY_FILE = os.path.join(STRATEGY_DIR, "buy_and_hold_summary.csv")
    ...
    plt.title(f"Buy & Hold Strategy: Total Return (%) by Asset and Interval (Top {TOP_N_ASSETS} Assets)", fontsize=16, pad=20, fontweight='bold')
    plt.xlabel("Interval", fontsize=12, fontweight='bold')
    plt.ylabel("Asset", fontsize=12, fontweight='bold')
    plt.xticks(rotation=45, ha='right')
    plt.yticks(rotation=0)
    plt.tight_layout()
    plt.savefig(os.path.join(STRATEGY_DIR, "buy_and_hold_heatmap.png"))
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(STRATEGY_DIR, "buy_and_hold_return_distribution.png"))
    plt.show()

    # --- Bar plot for each interval with robust y-axis scaling ---
    intervals = pivot.columns.tolist()
    for interval in intervals:
        data = pivot[interval].dropna().sort_values(ascending=False)
        num_assets = len(data)
